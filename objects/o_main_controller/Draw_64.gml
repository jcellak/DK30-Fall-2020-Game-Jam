	...
	for (var i = 0; i < array_length(current_menu); i ++)
	{
		if current_menu == key{
			...
			if i == cursor{
				...
				draw_text((r_screen-(margin*2)),t_screen+(margin*3)+(margin*i)-5,KeyInputDisplay(key_two[i]));
				draw_set_color(_col);
				draw_text(t_screen+(margin*7),t_screen+(margin*3)+(margin*i)-5,KeyInputDisplay(key_one[i]));
			}
			...
			if current_menu == opt and i < menu_items-2 {
				...
				draw_rectangle(l_screen+(margin*6)+(v_dist*vol[i]),t_screen+(margin)+(margin*i)+10,l_screen+(margin*6)+(v_dist*vol[i])+7,t_screen+(margin*2)+(margin*i),_volchk);
